...
void CStereoCalib::Calibration(CStereoCalibPara para)
{

	if (para.imagelist.size() % 2 != 0)
	{
		AfxMessageBox(_T("Error: the image list contains odd (non-even) number of elementsÅI"));
		return;
	}
	...
	Size boardSize;
	boardSize.width = para.nx;
	boardSize.height = para.ny;

	vector<vector<Point2f> > imagePoints[2];
	vector<vector<Point3f> > objectPoints;
	Size imageSize;

	int i, j, k, nimages = (int)para.imagelist.size() / 2;
	...
	for (i = j = 0; i < nimages; i++)
	{
		for (k = 0; k < 2; k++)
		{
			const string& filename = para.imagelist[i * 2 + k].GetBuffer(0);
			...
			{
				AfxMessageBox(_T("The Image") + para.imagelist[i * 2 + k] + _T(" has the size different from the first image size. Skipping the pair\n"));
		...
		if (k == 2)
		{
			goodImageList.push_back(para.imagelist[i * 2].GetBuffer(0));
			goodImageList.push_back(para.imagelist[i * 2 + 1].GetBuffer(0));
			j++;
		}
	}
	...
	for (i = 0; i < nimages; i++)
	{
		for (j = 0; j < boardSize.height; j++)
		{
			...
			{
				objectPoints[i].push_back(Point3f(k*para.squareSize, j*para.squareSize, 0));
			}
		}
	}
	...
	string inPath = para.prjName.GetBuffer(0);
	inPath += "\\intrinsics.yml";
	FileStorage fs(inPath, FileStorage::WRITE);
	if (fs.isOpened())
	{
	...
	{
		AfxMessageBox(_T("Error: can not save the intrinsic parametersÅI"));
	}
	...
	string exPath = para.prjName.GetBuffer(0);
	exPath += "\\extrinsics.yml";
	fs.open(exPath, FileStorage::WRITE);
	if (fs.isOpened())
	{
	...
	{
		AfxMessageBox(_T("Error: can not save the extrinsic parametersÅI"));
	}
...
CStereoCalibPara::CStereoCalibPara()
{
	// Default parameters
	nx = 9;
	ny = 6;
